namespace Olekstra.LikePharma.Client.Validators
{
    ...
    /// <summary>
    /// Валидатор, проверяющий что номер карты состоит из 19 цифр (проверка контрольного разряда не производится).
    /// </summary>
    /// <remarks>Пустые значения считаются правильными, проверка заполненности должна делаться другими валидаторами.</remarks>
    public class Digit19CardNumberValidator : ICardNumberValidator
    {
        /// <inheritdoc />
...
#pragma warning disable CS8602 // Dereference of a possibly null reference - выше value уже проверили на null.
            return (value.Length == 19 && value.All(c => c >= '0' && c <= '9'))
                ? ValidationResult.Success
                : new ValidationResult(ValidationMessages.Digit19CardNumberValidator_Failure);
        }
    }
}
